...
const DoctorRegistration = () => {
  ...
  if (name === "profilePhoto") {
    // Crucial: Extract strictly the first index raw binary file out of the FileList array
    setFormData((prevData) => ({
      ...prevData,
      profilePhoto: files && files.length > 0 ? files[0] : null
    }));
  } else {
    setFormData((prevData) => ({
      ...prevData,
  ...
  // };
    ...
    try {
      ...
      // 1. Appends regular text parameters safely without converting file objects to text string descriptions
      Object.keys(formData).forEach((key) => {
        if (key !== "profilePhoto" && formData[key] !== null && formData[key] !== undefined) {
          formDataToSend.append(key, formData[key]);
          console.log(`Adding text parameter - ${key}:`, formData[key]);
        }
      });

      // 2. Explicitly appends the isolated single raw binary image file object under the profilePhoto key
      if (formData.profilePhoto) {
        formDataToSend.append("profilePhoto", formData.profilePhoto);
        console.log("Attached raw binary image file object to 'profilePhoto' key.");
      } else {
        console.log("Warning: No profile photo selected to attach.");
      }

      // Log entries for terminal tracking
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/doctors/register`, {
        method: 'POST',
        body: formDataToSend,
        credentials: 'include', // Crucial to ensure HttpOnly session cookies are transmitted locally
      });

      console.log('Response status:', response.status);
      const responseText = await response.text();
      console.log('Raw response payload:', responseText);
      ...
      try {
        ...
        console.error('JSON Parse Error:', parseError);
        throw new Error('Invalid response payload layout received from server');
      }

      if (!response.ok) {
        throw new Error(data.error || 'Failed to complete doctor registration');
      }

      // 3. CACHE THE DYNAMIC REGISTRATION ATTRIBUTES FOR STATE PERSISTENCE
      // Stores user profile state parameters securely locally to maintain auth tracking
      localStorage.setItem('userType', 'doctor');
      localStorage.setItem('profileCompleted', 'true');
      
      if (data.doctor && data.doctor.id) {
        localStorage.setItem('doctorTypeId', data.doctor.id);
      }
      ...
      alert('Registration successful! Redirecting to doctor dashboard panel...');
      
      // Forward the provider straight into their working landing panel view route
      navigate('/doctor-dashboard');
    } catch (error) {
      console.error('Registration processing error error:', error);
      setError(error.message || 'There was an error submitting the form. Please try again.');
    } finally {
      setIsLoading(false);
    }
...
return (
    <div className="max-w-4xl mx-auto p-8 bg-[#f4f8ff] rounded-2xl shadow-md mt-10 mb-10">
      ...
      {/* FIXED: Added encType="multipart/form-data" to allow secure file uploads */}
